...
let ownAjax = null
function initOwnAajax () {
  ownAjax = new Promise((resolve, reject) => {
    let ajax = null
    ajax = axios.create({
      baseURL: "http://101.37.80.200:9000",
      timeout: 30000,
      headers: {
...
export default {
  ...
  getRouteInfoByPort (id) {
    return new Promise((resolve, reject) => {
      if (mock) {
        resolve(require('./mock/routeInfo-测试').content[0])
      } else {
        if (ownAjax == null) {
          initOwnAajax()
        }
