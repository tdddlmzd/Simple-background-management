    ...
    export default {
        ...
        methods: {
            searchList(abnormal){
                ...
                this.$axios.post(this.commonJs.localUrl +`/schedules/trace/getTerminalRecordList?pageNumber=${this.page.pageNo}&pageSize=${this.page.pageSize}`, query
                ,{
                    headers: {
            ...
            handleExportClick(){
                ...
                this.$axios({method: 'POST',url: this.commonJs.localUrl +`/schedules/trace/exportTerminalExcel?total=${this.$refs.refPagination.page.total}`,data:query,
                    responseType: 'blob',
                        headers: {
                            Authorization: `Bearer ${this.getAuthorization()}`,
                            AccessToken: this.getCookie("token").replace("Bearer","Jwt"),
