            ...
            <div class="parent">
                <div class="logLf">
                    ...
                    <span class="backgroundLogo">
                        <img src="@/assets/images/login/login_logo.png" class="backgroundK">
                    </span>
                </div>
                <el-form 
                    ref="loginForm" 
                ...
                >
                    ...
                    <div class="form-actions">
                        <label class="rememberme check mt-checkbox">
                            <el-checkbox v-model="loginForm.checked" style="color: #8290a3;">记住密码</el-checkbox>
                        </label>
                        <el-button type="primary" @click.native.prevent="handleLogin" style="width:100%;background:#3bafda; border-color:#3bafda; margin-top: 20px;">
                            登录
                        </el-button>
                    </div>
                </el-form>
...
<script>
    ...
    export default {
                ...
                this.$refs.loginForm.validate(valid => { // 表单验证
                    if (valid) {
                        ...
                        this.$axios.get(
                            ...
                            `/auth/token?username=${this.loginForm.username}&password=${this.loginForm.password}`
                        ).then(res => {
                            if(res.data.status !== 1){
                                ...
                                // 存储token
                                this.setCookie('authorization', JSON.stringify({authorization: res.data.content}), 1);
                                this.setCookie('token', 'Jwt ' + res.data.content);
        ...
        mounted(){
            ...
            if(this.getCookie('userName') && this.$localStorage.getItem('checked') === true){
