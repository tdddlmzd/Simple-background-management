<template>
    <div class="basicPortContent" ref="basicPortContent">
                    ...
                    <el-row>
                        ...
                        <el-col style="width:22%"> 
                            <el-form-item label="单号">
                                <el-input v-model="ruleForm.keywords" clearable style="width:100%" placeholder="请输入" ></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
        ...
        <div class="tablePage">
            <div class="buttonTop">
                <el-button size="small" class="teshunormal" @click="handleExportClick" :loading="isExportLoading">导出</el-button>
            </div>
            <el-table
                :data="tableData"
                style="width: 100%;"
            ...
            >
                ...
                <el-table-column  prop="carriercd" label="承运人"  :show-overflow-tooltip=true align="left" min-width="90">

                </el-table-column>
                <el-table-column  prop="resultStatus" label="查询结果" :show-overflow-tooltip=true align="left" min-width="80"> 
                    <template slot-scope="scope">
                        <span>{{scope.row.resultStatus == 1 ? "有数据" : scope.row.resultStatus == 1005 ? "未订阅" : "无数据"}}</span>
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            searchList(abnormal){
                ...
                this.$axios.post(this.commonJs.localUrl +`/schedules/air/getHistoryList?pageNumber=${this.page.pageNo}&pageSize=${this.page.pageSize}`, query
                ,{
                    headers: {
            ...
            handleExportClick(){
                ...
                that.isExportLoading = true
                that.exportSocket('airRecord')
                this.$axios({method: 'POST',url: this.commonJs.localUrl +`/schedules/air/exportRecordExcel?total=${this.$refs.refPagination.page.total}`,data:query,
                    responseType: 'blob',
                        headers: {
                            Authorization: `Bearer ${this.getAuthorization()}`,
                            AccessToken: this.getCookie("token").replace("Bearer","Jwt"),
        ...
        mounted(){
            ...
            this.$bus.on('clickMenu', content => {
                ...
                this.ruleForm.keywords = "", //单号，
                this.ruleForm.carriercd ="", //承运人
        ...
        computed: {
            exportValue(){
                return sessionStorage.getItem('exportStorage')
            },
        },
        watch: {
            exportValue : {
                ...
                handler(newVal){
                    if(newVal){
                        var newArr = JSON.parse(sessionStorage.getItem('exportStorage'))
                        if(newArr.length > 0){
                            for (let i = 0; i < newArr.length; i++) {
                                if(newArr[i] == 'airRecord'){
